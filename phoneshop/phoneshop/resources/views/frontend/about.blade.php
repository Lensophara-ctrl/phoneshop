...
<div class="about-hero">
    <div class="about-hero-content">
        ...
        <p>{{ $settings['about_hero_subtitle'] ?? 'Your trusted partner in mobile technology, delivering quality products and exceptional service since day one.' }}</p>
    </div>
</div>
...
<div class="row mb-5">
    <div class="col-lg-6 mb-4 mb-lg-0">
        <h2 class="fw-bold mb-4">{{ $settings['about_story_title'] ?? 'Our Story' }}</h2>
        @if(isset($settings['about_story_content']) && $settings['about_story_content'])
            {!! nl2br(e($settings['about_story_content'])) !!}
        @else
            <p class="text-muted mb-3" style="font-size: 1.05rem; line-height: 1.8;">
                Welcome to <strong>{{ $settings['store_name'] ?? 'PhoneShop' }}</strong>, your premier destination for the latest smartphones and mobile accessories. 
    ...
    <div class="col-lg-6">
        <div class="position-relative" style="height: 100%; min-height: 300px;">
            <div style="background: linear-gradient(135deg, rgba(79, 70, 229, 0.1) 0%, rgba(124, 58, 237, 0.1) 100%); 
                ...
                @endphp
                <i class="fa-solid {{ $storeIcon }}" style="font-size: 10rem; color: #4f46e5; opacity: 0.2;"></i>
            </div>
        </div>
    </div>
</div>
...
<div class="stats-section">
    <div class="row">
        <div class="col-6 col-md-3">
            <div class="stat-item">
                <div class="stat-number">{{ $settings['about_stat_customers'] ?? '1000+' }}</div>
                <div class="stat-label">Happy Customers</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-item">
                <div class="stat-number">{{ $settings['about_stat_products'] ?? '500+' }}</div>
                <div class="stat-label">Products</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-item">
                <div class="stat-number">{{ $settings['about_stat_authentic'] ?? '100%' }}</div>
                <div class="stat-label">Authentic</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-item">
                <div class="stat-number">{{ $settings['about_stat_support'] ?? '24/7' }}</div>
...
<!-- Team/CV Section -->
@if(isset($settings['about_cv_enabled']) && $settings['about_cv_enabled'] == '1')
<div class="team-section">
                ...
                <div class="card-body p-5">
                    <div class="row">
                        <div class="col-md-4 text-center mb-4 mb-md-0">
                            @if(isset($settings['about_cv_photo']) && $settings['about_cv_photo'])
                                <img src="{{ asset('storage/' . $settings['about_cv_photo']) }}" 
                                     alt="{{ $settings['about_cv_name'] ?? 'Team Member' }}" 
                            ...
                            @endif
                            <h4 class="fw-bold mb-1">{{ $settings['about_cv_name'] ?? 'Team Member' }}</h4>
                            <p class="text-primary fw-semibold mb-3">{{ $settings['about_cv_position'] ?? 'Position' }}</p>
                            
                            @if(isset($settings['about_cv_email']) && $settings['about_cv_email'])
                                <div class="mb-2">
                                    <i class="fa-solid fa-envelope text-muted me-2"></i>
                                    <small>{{ $settings['about_cv_email'] }}</small>
                                </div>
                            @endif
                            
                            @if(isset($settings['about_cv_phone']) && $settings['about_cv_phone'])
                                <div class="mb-2">
                                    <i class="fa-solid fa-phone text-muted me-2"></i>
                                    <small>{{ $settings['about_cv_phone'] }}</small>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="col-md-8">
                            ...
                            <p class="text-muted mb-4" style="line-height: 1.8;">
                                {{ $settings['about_cv_bio'] ?? 'Professional bio goes here.' }}
                            </p>
                            
                            @if(isset($settings['about_cv_skills']) && $settings['about_cv_skills'])
                                <h5 class="fw-bold mb-3">
                                    <i class="fa-solid fa-code text-primary me-2"></i>Skills
                                </h5>
                                <div class="mb-4">
                                    @foreach(explode(',', $settings['about_cv_skills']) as $skill)
                            ...
                            @if(isset($settings['about_cv_education']) && $settings['about_cv_education'])
                                <h5 class="fw-bold mb-3">
                                    <i class="fa-solid fa-graduation-cap text-primary me-2"></i>Education
                                </h5>
                                <p class="text-muted mb-4" style="line-height: 1.8;">
                                    {{ $settings['about_cv_education'] }}
                                </p>
                            @endif
                            
                            @if(isset($settings['about_cv_experience']) && $settings['about_cv_experience'])
                                <h5 class="fw-bold mb-3">
                                    <i class="fa-solid fa-briefcase text-primary me-2"></i>Experience
                                </h5>
                                <p class="text-muted" style="line-height: 1.8;">
                                    {{ $settings['about_cv_experience'] }}
                                </p>
                            @endif
                        </div>
                    </div>
...
<div class="text-center mt-5 pt-5 border-top">
    ...
    <a href="{{ route('shop.home') }}" class="btn btn-primary btn-lg px-5 rounded-pill">
