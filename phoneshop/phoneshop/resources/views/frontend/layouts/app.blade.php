...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/netflix-carousel.css') }}">
</head>
<body>
...
<nav class="navbar navbar-expand-lg sticky-top">
    <div class="container">
        <a class="navbar-brand" href="{{ route('shop.home') }}">
            ...
            @endphp
            <i class="fa-solid {{ $storeIcon }} me-2"></i>
            {{ $storeName }}
        </a>
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('shop.home') }}">{{ __('messages.home') }}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('shop.about') }}">{{ __('messages.about_us') }}</a>
                </li>
            </ul>
            
            <form class="d-flex me-3" action="{{ route('shop.search') }}" method="GET" role="search">
                <div class="input-group">
                    <input class="form-control" type="search" name="q" placeholder="{{ __('messages.search_products') }}" value="{{ request('q') }}" style="border-radius: 0.5rem 0 0 0.5rem;">
            ...
            <ul class="navbar-nav align-items-center">
                ...
                <li class="nav-item dropdown me-2">
                    <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="languageDropdown" role="button" data-bs-toggle="dropdown">
                        ...
                        @endphp
                        <span class="me-2">{{ $flags[$currentLocale] ?? '🇬🇧' }}</span>
                        <span class="d-none d-md-inline">{{ $names[$currentLocale] ?? 'English' }}</span>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2">
                        <li>
                            <form action="{{ route('language.switch') }}" method="POST" class="dropdown-item-form">
                                ...
                                <button type="submit" class="dropdown-item py-2 {{ $currentLocale === 'en' ? 'active' : '' }}">
                                    🇬🇧 English
                                </button>
                            </form>
                        </li>
                        <li>
                            <form action="{{ route('language.switch') }}" method="POST" class="dropdown-item-form">
                                ...
                                <button type="submit" class="dropdown-item py-2 {{ $currentLocale === 'km' ? 'active' : '' }}">
                                    🇰🇭 ខ្មែរ
                                </button>
                            </form>
                        </li>
                        <li>
                            <form action="{{ route('language.switch') }}" method="POST" class="dropdown-item-form">
                                ...
                                <button type="submit" class="dropdown-item py-2 {{ $currentLocale === 'zh' ? 'active' : '' }}">
                                    🇨🇳 中文
                                </button>
                            </form>
                        </li>
                ...
                <li class="nav-item me-3">
                    <a href="{{ route('shop.cart') }}" class="btn btn-light position-relative rounded-circle p-2" style="width: 42px; height: 42px;">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown">
                            @if(Auth::user()->profile_image)
                                <img src="{{ asset('storage/' . Auth::user()->profile_image) }}" alt="{{ Auth::user()->name }}" class="rounded-circle me-2" style="width: 32px; height: 32px; object-fit: cover;">
                            @else
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2">
                            <li><a class="dropdown-item py-2" href="{{ route('profile.show') }}"><i class="fa-solid fa-user me-2"></i>My Profile</a></li>
                            @if(Auth::user()->role !== 'admin')
                                <li><a class="dropdown-item py-2" href="{{ route('customer.orders') }}"><i class="fa-solid fa-shopping-bag me-2"></i>My Orders</a></li>
                            @endif
                            @if(Auth::user()->role === 'admin')
                                <li><a class="dropdown-item py-2" href="{{ route('dashboard') }}"><i class="fa-solid fa-gauge me-2"></i>Admin Panel</a></li>
                            @endif
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-3 col-md-6 footer-section">
                <h5 class="footer-title">{{ $settings['store_name'] ?? 'About PhoneShop' }}</h5>
                <p class="text-muted" style="font-size: 0.95rem; line-height: 1.6;">
                    {!! $settings['footer_about'] ?? 'Your trusted destination for premium smartphones and accessories with the best prices and quality assurance.' !!}
                </p>
            </div>

            <div class="col-lg-3 col-md-6 footer-section">
                <h5 class="footer-title">{{ $sectionTitles['quick_links'] ?? 'Quick Links' }}</h5>
                @foreach($quickLinks as $link)
                    <a href="{{ $link['url'] ?? '#' }}" class="footer-link">
                        <i class="fa-solid {{ $link['icon'] ?? 'fa-link' }} me-1"></i>{{ $link['title'] ?? 'Link' }}
                    </a>
                @endforeach
            </div>

            <div class="col-lg-3 col-md-6 footer-section">
                <h5 class="footer-title">{{ $sectionTitles['support'] ?? 'Support' }}</h5>
            ...
            <div class="col-lg-3 col-md-6 footer-section">
                <h5 class="footer-title">{{ $sectionTitles['follow_us'] ?? 'Follow Us' }}</h5>
                ...
                <div class="social-links">
                    @if($settings['footer_facebook'] ?? false)
                        <a href="{{ $settings['footer_facebook'] }}" class="social-icon" title="Facebook" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-facebook-f"></i>
                        </a>
                    @endif
                    @if($settings['footer_twitter'] ?? false)
                        <a href="{{ $settings['footer_twitter'] }}" class="social-icon" title="Twitter" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-twitter"></i>
                        </a>
                    @endif
                    @if($settings['footer_instagram'] ?? false)
                        <a href="{{ $settings['footer_instagram'] }}" class="social-icon" title="Instagram" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-instagram"></i>
                        </a>
                    @endif
                    @if($settings['footer_linkedin'] ?? false)
                        <a href="{{ $settings['footer_linkedin'] }}" class="social-icon" title="LinkedIn" target="_blank" rel="noopener noreferrer">
        ...
        <div class="footer-bottom">
            <p class="mb-2">
                <i class="fa-solid fa-copyright me-1"></i>{!! $settings['footer_copyright'] ?? '&copy; ' . date('Y') !!} {{ $settings['footer_text'] ?? 'PhoneShop. All rights reserved.' }}
            </p>
...
<script src="{{ asset('js/netflix-carousel.js') }}"></script>

<!-- Live Chat Widget -->
@include('frontend.partials.live-chat')
</body>
</html>
