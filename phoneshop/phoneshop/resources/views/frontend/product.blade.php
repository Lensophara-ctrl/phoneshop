...
<div class="container py-5">
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('shop.category', $phone->category_id) }}">{{ $phone->category->name }}</a></li>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-6">
            <div class="product-gallery">
                <div class="main-image-container">
                    @if($phone->image)
                        <img src="{{ asset('storage/' . $phone->image) }}" alt="{{ $phone->name }}" class="main-image" id="mainImage">
                    @else
                ...
                @if($phone->detail_images && count($phone->detail_images) > 0)
                    <div class="thumbnail-gallery">
                        ...
                        @if($phone->image)
                            <div class="thumbnail-item active" onclick="changeImage('{{ asset('storage/' . $phone->image) }}', this)">
                                <img src="{{ asset('storage/' . $phone->image) }}" alt="Main">
                            </div>
                        @endif

                        <!-- Detail images thumbnails -->
                        @foreach($phone->detail_images as $detailImage)
                            <div class="thumbnail-item" onclick="changeImage('{{ asset('storage/' . $detailImage) }}', this)">
                                <img src="{{ asset('storage/' . $detailImage) }}" alt="Detail {{ $loop->iteration }}">
                            </div>
                        @endforeach
                    </div>
                @endif
        ...
        <div class="col-lg-6">
            <div class="product-info-card">
                <span class="badge bg-primary-subtle text-primary mb-3">{{ $phone->category->name }}</span>
                
                <h1 class="fw-bold mb-3">{{ $phone->name }}</h1>

                <div class="mb-4">
                    @if($phone->qty > 10)
                        <span class="stock-indicator in-stock">
                            <i class="fa-solid fa-circle-check"></i>
                            In Stock ({{ $phone->qty }} available)
                        </span>
                    @elseif($phone->qty > 0)
                        <span class="stock-indicator low-stock">
                            <i class="fa-solid fa-exclamation-triangle"></i>
                            Low Stock (Only {{ $phone->qty }} left)
                        </span>
                    @else
                ...
                <div class="price-display mb-4">${{ number_format($phone->price, 2) }}</div>

                @if($phone->description)
                    <div class="mb-4">
                        <h5 class="fw-bold mb-3">Description</h5>
                        <p class="text-muted" style="line-height: 1.8;">{{ $phone->description }}</p>
                    </div>
                @endif
                ...
                <div class="d-grid gap-3">
                    @if($phone->qty > 0)
                        <a href="{{ route('shop.add', $phone->id) }}" class="btn btn-add-cart">
                    ...
                    <a href="{{ route('shop.home') }}" class="btn btn-outline-secondary btn-lg rounded-pill">
