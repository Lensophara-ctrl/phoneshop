    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="row row-cols-2 row-cols-md-3 row-cols-xl-4 g-3 product-grid-container mb-4" id="products-grid" style="overflow-y: auto;">
                ...
                <div class="col product-item" data-category="{{ $product->category_id }}" data-name="{{ strtolower($product->name) }}">
                    <div class="card h-100 modern-card modern-product-card {{ $product->qty <= 0 ? 'opacity-50' : '' }}" 
                         onclick="{{ $product->qty > 0 ? 'addToCart('.json_encode($product).')' : 'alert(\'Out of stock!\')' }}">
                        
                        <span class="stock-badge {{ $product->qty > 10 ? 'bg-success text-white' : ($product->qty > 0 ? 'bg-warning text-dark' : 'bg-danger text-white') }}">
                        ...
                        <div class="card-body p-3 pt-0">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-2">
                                <span class="text-primary fw-bold h6 mb-0">${{ number_format($product->price, 2) }}</span>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const EXCHANGE_RATE = {{ $exchange_rate ?? 4100 }};
        let cart = [];
        ...
        function fetchTodaySales() {
            fetch('{{ route("sales.latest") }}?filter=today', {
                headers: {
