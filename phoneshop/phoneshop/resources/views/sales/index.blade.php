    ...
    <div class="card-body p-3">
        <form action="{{ route('sales.index') }}" method="GET" class="row g-3 align-items-end">
            ...
            <div class="col-md-4">
                ...
                <input type="date" name="date" class="form-control border-0 bg-light" value="{{ request('date') }}" onchange="this.form.submit()">
            </div>
            <div class="col-md-4 d-flex gap-2">
                @if(request()->hasAny(['filter', 'date']))
                    <a href="{{ route('sales.index') }}" class="btn btn-light w-100 fw-bold">
        ...
        <div class="card p-4 border-0 shadow-sm bg-primary text-white">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <div class="h4 fw-bold mb-0" id="total-orders">{{ $sales->count() }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card p-4 border-0 shadow-sm">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <div class="h4 fw-bold mb-0" id="total-revenue">${{ number_format($sales->sum('total_price'), 2) }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card p-4 border-0 shadow-sm">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <div class="h4 fw-bold mb-0" id="unique-customers">{{ $sales->unique('user_id')->count() }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card p-4 border-0 shadow-sm">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <div class="h4 fw-bold mb-0" id="products-sold">{{ $sales->sum(fn($sale) => $sale->items->sum('qty')) }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    (function() {
        ...
        const fetchLatestSales = async () => {
            ...
            try {
                ...
                const response = await fetch(`{{ route('sales.latest') }}?${queryString}`, {
                    headers: {
